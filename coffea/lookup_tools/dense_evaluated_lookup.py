...
class dense_evaluated_lookup(lookup_base):
    ...
    def _evaluate(self, *args):
        ...
        if self._dimension == 1:
            indices.append(np.clip(searchsorted_wrapped(self._axes, args[0], side='right') - 1, 0, self._values.shape[0] - 1))
        else:
            for dim in range(self._dimension):
                indices.append(np.clip(searchsorted_wrapped(self._axes[dim], args[dim], side='right') - 1,
                                       0, self._values.shape[len(self._axes) - dim - 1] - 1))
        indices.reverse()
        if USE_CUPY:
            var = np.asnumpy(args[self._feval_dim])
        else:
            var = args[self._feval_dim]
        return np.asarray(numba_apply_1d(self._values[tuple(indices)], var))
